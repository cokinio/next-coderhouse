...
export async function POST(req) {
    try {
        ...
        if (!title || !image || !price || !category || !stock) {
            return NextResponse.json({ error: 'Error: fatan datos para crear producto' }, { status: 400 })
        }

    
        let wasProductAddedSuccesfully = await productsModel.create(product)
        return NextResponse.json({ status: "Success", message: `Producto agregado con exito con ID:${wasProductAddedSuccesfully[1]}`});
